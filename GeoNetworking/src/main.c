...
static struct option const long_options[] =
{
	...
	}, {"help", no_argument, 0, 'h'}, {"version", no_argument, 0, 'V'}, {NULL, 0, NULL, 0}
};
...
static void init_conf(void){
	struct itsnet_position_information init_position;

	// All node multicast radius
	geoDst.AN_Mcast_radius = AllNodeMcast_Radius_parse();
	// MLD group address
        geoDst.MLD_group = MLD_GroupAddress_parse();
	// MLD group address radius
	geoDst.MLD_group_radius=MLD_GroupAddress_Radius_parse();
	// Initial Position
	init_position.time_stamp = 0;
	init_position.latitude = InitialLatitude_parse() * 8000000;
	init_position.longitude = InitialLongitude_parse() * 8000000;
	init_position.altitude = InitialAltitude_parse();
	init_position.speed = InitialSpeed_parse();
	init_position.heading = InitialHeading_parse();
	position_data_update(init_position);
