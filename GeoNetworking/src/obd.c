...
static int open_uart(const char *dev)
{
	...
	fd = open(dev, O_RDWR); 	
	if (fd == -1){ 			
		perror("Can't Open Serial Port for Bluetooth"); 
        	return -1;
	}
	else{
		//printf("1");	
...
static void read_uart( int fd, char *buff, int bufflen)
{
	...
	while ((nread = read(fd, buff + tmpLen, 512)) > 0){ 
		tmpLen += nread;
		//determined by your project, detect it!!!
		if (buff[tmpLen - 1] == '>'){ 
			break;
		}
	}
...
static int init_obd( int fd)
{
	...
	while ( flag != 4) {
		...
		case 0:{
				...
				else	perror("write buf_atz error!\n");
				break;
			}
		...
		case 1:{		
				...
				else	perror("write buf_ate0 error");	
				break;	
			}
		...
		case 2:{
				...
				else	perror("write buf_atsp0 error");
				break;
			}
		...
		case 3:{
				...
				if( write(fd, buf_connect, sizeof(buf_connect)) == sizeof(buf_connect)){
					...
					printf("%s",buff);
					printf("%s",buff+13);
					if ( strncmp(buff + 13, "41 00", 5) == 0 ){  // || (strncmp(buff, "SEARCH", 6)
						printf("CONNECT...\n");
						flag = 4;
					}
					else{
				...
				else	perror("write buf_connect error");
				break;
			}
...
static int Engine_rpm_formula(int fd, const char *commmand)
{
	int rpm = 0;
	char buff[64] = "";
...
static int Vehicle_speed_formula(int fd, const char *command)
{
	int speed = 0;
	char buff[64] = "";
...
static int Engine_coolant_temperature_formula(int fd, const char *command)
{
	int temp = 0;
	char buff[64] = "";
	int first_byte = 0;
...
static int number_of_DTC_formula(int fd, const char *commmand)
{
	int num_DTC = 0;
	char buff[64] = "";
	int first_byte = 0;
...
//		return OBD_ERROR;
//}
static int intake_air_temperature_formula(int fd, const char *command)
{
	int temperature = 0;
	char buff[64] = "";
	int first_byte = 0;
...
//		return OBD_ERROR;
//}
static int fuel_system1_status_formula(int fd, const char *command)
{
	int status = 0;
	char buff[64] = "";
...
static int Intake_manifold_absolute_pressure_formula(int fd, const char *command)
{
	int pressure = 0;
	char buff[64] = "";
...
static float LTFT_formula(int fd, const char *command, int len_cmd)
{
	float ltft;
	char buff[64] = "";
	int first_byte;
...
static void read_obd( int fd, struct OBDInfo *p)
{
	float ltft = 0;
	...
	while (1)
	{
		...
		rep_maf = (iat + 273.15) / (float)(rpm * imap) * 12000.0 / (VE * ED) * (R / MM) ; // 1/maf   Reciprocal of maf 
        	mpg = 7.107 * vss_now * rep_maf * (1 + ltft / 100.0);// mile/gallon
		if (mpg != 0) {
...
{
	int fd;
    	const char *dev = OBD_DEV_PATH;
	//printf("OBD thread start!\n");
	fd = open_uart(dev);
    	if (init_uart( fd)){
		printf("Start On-Board Diagnostic module ...\n");
        	init_obd( fd);
